...
echo "Getting metadata for Google Chrome repository..."
meta_data=$(curl "$CHROME_URL")

echo "Downloading the latest Chrome binary..."
chrome_url=$(echo "$meta_data" | jq -r '.channels.Stable.downloads.chrome[] | select(.platform=="linux64") | .url')
...
chromedriver_url=$(echo "$meta_data" | jq -r '.channels.Stable.downloads.chromedriver[] | select(.platform=="linux64") | .url')
